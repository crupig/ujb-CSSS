...
from common import reorg_output_file
import tasks

def deduplicate_tmp_file(input_file):
    if not os.path.exists(input_file):
        raise FileNotFoundError(f"The file {input_file} does not exist.")
    if not input_file.endswith('.json.tmp'):
        print(f"Warning: {input_file} is not a .json.tmp file. Cannot deduplicate.")
        return
...
def get_model_answers(
    ...
    test_file_headers = json.load(open("./code_ujb/constants/taskid_testfilehead.json"))
    ...
    for task in tqdm(all_tasks):
        ...
        if gen_mode == "chat":
            test_file_header = test_file_headers[str(task["task_idx"])]
            instruction = """Given the following coding task specifications, provide a java test suite with up to 10 test methods with the goal of fully testing the solutions for the coding task.
...
{}
""".format(task['question'], test_file_header)
            conv = get_conversation_template(model_id)
        ...
        deduplicate_tmp_file(save_generations_path)
...
if __name__ == "__main__":
    ...
    ids_to_keep = None
    if args.all_ids_dict is not None:
        ids_to_keep = ast.literal_eval(args.all_ids_dict)["ujb"][args.split]
        ids_to_keep = [int(_id.split('-')[-1]) for _id in ids_to_keep]
    
    save_generations_path = args.save_generations_path.split(".json")[0] + f"-{os.getpid()}.json"
    run_generate(
        ...
        ids_subset=ids_to_keep
    )

    if args.save_generations_path:
        fail_messages = reorg_output_file(save_generations_path, args.num_samples)

        if len(fail_messages) > 0:
            print("Some tasks failed during generation:")
